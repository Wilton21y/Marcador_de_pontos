  ...
  const bt_m1A = document.querySelectorAll('.bt')[2].addEventListener('click', () => {
   document.querySelectorAll('h3')[0].innerHTML = --contadorPa;
  })
  const bt_m1V = document.querySelectorAll('.bt')[3].addEventListener("click", () => {
   document.querySelectorAll('h3')[1].innerHTML = --contador;
  });

  /* Botões dos cronômetro */
  const ini = document.querySelectorAll(".bt")[4].addEventListener('click', play);

  const stop = document.querySelectorAll('.bt')[5].
  addEventListener('click', pauser_time);

  /* button reset time */
  const reset = document.querySelectorAll('.bt')[6].addEventListener('click', reiniciar);
  ...
  function cronometro() {
   ...
   tempo = document.getElementById('marcar').innerText = twornum(hr) + ":" + twornum(min) + ":" + twornum(sec);
  }
